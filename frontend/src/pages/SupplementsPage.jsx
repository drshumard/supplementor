  ...
  return (
    <div className="p-10 max-w-[1560px] mx-auto">
      <div className="flex items-center justify-between mb-10">
        <div>
          <h1 className="text-2xl font-semibold tracking-[-0.02em] text-[#0B0D10]">Master Supplement List</h1>
          <p className="text-sm text-muted-foreground mt-1.5">{total} supplement{total !== 1 ? 's' : ''}</p>
        </div>
        <Button onClick={openAdd} className="gap-2.5 h-11 px-6 bg-[hsl(187,79%,23%)] hover:bg-[hsl(187,79%,28%)] text-white font-semibold shadow-sm" data-testid="admin-supplements-add-button">
      ...
      <div className="relative max-w-[360px] mb-8">
        ...
        <Input placeholder="Search supplements..." value={search} onChange={(e) => setSearch(e.target.value)}
          data-testid="admin-supplements-search-input" className="pl-11 h-11" />
      </div>

      <div className="rounded-xl border bg-card shadow-[var(--shadow-sm)] overflow-hidden" data-testid="admin-supplements-table">
        <Table>
          <TableHeader>
            <TableRow className="hover:bg-transparent bg-[#FAFBFB]">
              <TableHead className="text-[11px] font-bold tracking-[0.1em] uppercase text-muted-foreground py-4 px-5">Name</TableHead>
              <TableHead className="text-[11px] font-bold tracking-[0.1em] uppercase text-muted-foreground py-4">Brand</TableHead>
              <TableHead className="text-[11px] font-bold tracking-[0.1em] uppercase text-muted-foreground py-4 text-center">Size</TableHead>
              ...
              <TableHead className="text-[11px] font-bold tracking-[0.1em] uppercase text-muted-foreground py-4 text-center">Btls/Mo</TableHead>
              <TableHead className="text-[11px] font-bold tracking-[0.1em] uppercase text-muted-foreground py-4 text-center">Status</TableHead>
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="max-w-[600px] max-h-[85vh] overflow-y-auto p-7">
          ...
          <div className="grid gap-5 py-4">
            <div className="grid grid-cols-2 gap-5">
              <div className="space-y-2 col-span-2">
                ...
                <Input value={editData.supplement_name} onChange={(e) => setEditData({...editData, supplement_name: e.target.value})} className="h-11" />
              </div>
              <div className="space-y-2">
                <Label className="text-sm font-semibold">Company / Brand</Label>
                <Input value={editData.company} onChange={(e) => setEditData({...editData, company: e.target.value})} className="h-11" />
              </div>
              <div className="space-y-2">
                <Label className="text-sm font-semibold">Unit Type</Label>
                <Input value={editData.unit_type} onChange={(e) => setEditData({...editData, unit_type: e.target.value})} className="h-11" placeholder="caps, ml, scoops" />
              </div>
              <div className="space-y-2">
                <Label className="text-sm font-semibold">Units Per Bottle</Label>
                <Input type="number" value={editData.units_per_bottle} onChange={(e) => setEditData({...editData, units_per_bottle: e.target.value})} className="h-11 font-mono" />
              </div>
              <div className="space-y-2">
                <Label className="text-sm font-semibold">Cost Per Bottle ($)</Label>
                <Input type="number" step="0.01" value={editData.cost_per_bottle} onChange={(e) => setEditData({...editData, cost_per_bottle: e.target.value})} className="h-11 font-mono" />
              </div>
              <div className="space-y-2">
                <Label className="text-sm font-semibold">Default Qty Per Dose</Label>
                <Input type="number" value={editData.default_quantity_per_dose} onChange={(e) => setEditData({...editData, default_quantity_per_dose: e.target.value})} className="h-11 font-mono" />
              </div>
              <div className="space-y-2">
                <Label className="text-sm font-semibold">Default Freq Per Day</Label>
                <Input type="number" value={editData.default_frequency_per_day} onChange={(e) => setEditData({...editData, default_frequency_per_day: e.target.value})} className="h-11 font-mono" />
              </div>
              <div className="space-y-2 col-span-2">
                <Label className="text-sm font-semibold">Default Dosage Display</Label>
                <Input value={editData.default_dosage_display} onChange={(e) => setEditData({...editData, default_dosage_display: e.target.value})} className="h-11" placeholder="e.g., 2 caps 3x/day" />
              </div>
              <div className="space-y-2 col-span-2">
                <Label className="text-sm font-semibold">Default Instructions</Label>
                <Input value={editData.default_instructions} onChange={(e) => setEditData({...editData, default_instructions: e.target.value})} className="h-11" placeholder="e.g., With food" />
              </div>
              <div className="space-y-2">
                <Label className="text-sm font-semibold">Bottles Per Month (Manual)</Label>
                <Input type="number" step="0.01" value={editData.bottles_per_month} onChange={(e) => setEditData({...editData, bottles_per_month: e.target.value})} className="h-11 font-mono" placeholder="Override calc" />
              </div>
              <div className="space-y-2 col-span-2">
          ...
          <DialogFooter className="gap-3 mt-2">
            <Button variant="outline" onClick={() => setEditOpen(false)} className="h-10 px-5">Cancel</Button>
            <Button onClick={handleSave} disabled={saving} className="h-10 px-6 bg-[#0B0D10] hover:bg-[#1a1d21] text-white font-semibold">{saving ? 'Saving...' : (editId ? 'Update' : 'Add')}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <AlertDialogContent className="p-7">
          ...
          <AlertDialogFooter className="mt-6 gap-3">
            ...
            <AlertDialogAction onClick={handleDelete} className="bg-red-600 text-white hover:bg-red-700 h-10 px-5 font-semibold">Delete</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
